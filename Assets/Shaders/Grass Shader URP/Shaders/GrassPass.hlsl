...
//configuraçao da Posição e rotaçao
[maxvertexcount(6)]
void LitPassGeom(triangle Varyings input[3], inout TriangleStream<Varyings> outStream)
{
    ...
    Varyings o3 = input[0];
    float3 o3Pos = (basePos + rotatedTangent * _Base + rotatedNormal*clamp(_MaxHeight * randomHeight,_MinHeight,_MaxHeight));
    o3.positionCS = TransformWorldToHClip(o3Pos);

    Varyings o4 = input[0];
    float3 o4Pos = (basePos - rotatedTangent * _Base + rotatedNormal * clamp(_MaxHeight * randomHeight, _MinHeight, _MaxHeight));
